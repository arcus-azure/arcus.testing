...
<Tabs>
  <TabItem value="xunit" label="xUnit" default>
    ...
    [Collection("DataFactory")]
    public class OtherDataFactoryTests
    {
        public OtherDataFactoryTests(DataFactoryFixture fixture)
        {
        }
    }
    ```
  ...
  <TabItem value="expecto" label="Expecto">
    ...
    [<EntryPoint>]
    let main args = async {
        let! session = TemporaryDataFlowDebugSession.StartDebugSessionAsync(...) |> Async.AwaitTask
        let tests = TestList ([
            myDataFactoryTests session
        ])
