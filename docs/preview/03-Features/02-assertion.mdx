...
:::info[Why use the `AssertXslt` to load something that is already available with `XsltCompiledTransform.Load`?]
...
:::info[Why use the `AssertXslt.Load` to load something that is already available with `XslCompiledTransform.Load`?]
