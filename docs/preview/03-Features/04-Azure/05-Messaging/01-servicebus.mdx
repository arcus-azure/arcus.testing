...
await using var topic = await TemporaryTopic.CreateIfNotExistsAsync(
  "<fully-qualified-namespace>", "<topic-name>", logger, cancellationToken);
```
...
await topic.AddSubscriptionAsync("<subscription-name>", cancellationToken);
```
...
await using var sub = await TemporaryTopicSubscription.CreateIfNotExistsAsync(
    "<fully-qualified-namespace>", "<topic-name>", "<subscription-name>", logger, cancellationToken);
```
...
await sub.AddRuleIfNotExistsAsync("<rule-name>", filter, cancellationToken);
```
...
await using var queue = await TemporaryQueue.CreateIfNotExistsAsync(
  "<fully-qualified-namespace>", "<queue-name>", logger, cancellationToken);
```
